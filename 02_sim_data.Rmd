...
I always use the `tidyverse` for `r glossary("data wrangling")`", so I'll create a data table using the `tibble()` function. We need to know what condition each subject is in, so set the first `A_sub_n` values to "A" and the next `B_sub_n` values to "B". Then set the score to the `A_scores` `r glossary("concatenate", "concatenated")` to the `B_scores`.
...
```{block, type="info"}
The code above "effect codes" condition, which we will use later in a mixed effect model. You can learn more about [coding schemes](https://debruine.github.io/posts/coding-schemes/) here.
```
