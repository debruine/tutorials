...
We're going to create a `r glossary("function")` that reports a p-value in APA style, named `report_p`. It will take two `r glossary("argument", "arguments")`, the p-value (`p`) and the number of digits to round to (`digits`). 
...
APA style omits the leading zero and pads the number out to three digits. We can do this by converting our rounded p-value into a `r glossary("character")` string, replacing the string "0." with ".", and making sure to pad the right side with enough zeros. The `stringr` package has useful functions for this.
...
```{r, echo = FALSE, fig.cap="Insert Roxygen Skeleton"}
...
If you don't have `r glossary("git")` installed on your computer, don't have it integrated with RStudio, and/or don't have a github account, follow the instructions in Appendix \@ref(#git).

### Set up git for this project

If you aren't already using `r glossary("version control")` for this project, make sure all of your files are saved and type `usethis::use_git()` into the console. Choose Yes to commit and Yes to restart R.
...
```{r, echo = FALSE, fig.cap="Enable GitHub Pages"}
