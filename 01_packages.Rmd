...
You will need to install the following `r glossary("package", "community-written functions")`s:
...
You'll see the following output, and a new RStudio `r glossary("project", "way to organise related files")` will open up. You can close the old window now and just work in this project.
...
We're going to create a `r glossary("function", "a named section of code that can be reused")` that reports a p-value in APA style, named `report_p`. It will take two `r glossary("arguments", "a variable that provides input to a function")`, the p-value (`p`) and the number of digits to round to (`digits`). 
...
APA style omits the leading zero and pads the number out to three digits. We can do this by converting our rounded p-value into a `r glossary("character", "a data type representing text")` string, replacing the string "0." with ".", and making sure to pad the right side with enough zeros. The `stringr` package has useful functions for this.
...
```{block type="info"}
One function you can't preface with the package name is the [pipe](#pipes). While you're testing your function, load the pipe by typing `library(magrittr)` in the console.
```
...
If you aren't already using `r glossary("version control", "a way to save a record of changes to your files")` for this project, make sure all of your files are saved and type `usethis::use_git()` into the console. Choose Yes to commit and Yes to restart R.
